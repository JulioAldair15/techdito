...
(function () {
    ...
    // ------------------------------------------------------------------------
    const ESTADOS = [
        ['A', 'DÍA ASISTIDO'], ['DT', 'DOMINGO TRABAJADO'], ['FT', 'FERIADO TRABAJADO'],
        ['LG', 'LICENCIA CON GOCE DE HABER'], ['DM', 'DESCANSO MÉDICO'], ['V', 'VACACIONES'],
        ['LSG', 'LICENCIA SIN GOCE DE HABER'], ['F', 'FALTA'], ['R', 'RENUNCIÓ'],
        ['SU', 'SUSP. PERFECTA LABORES'], ['CE', 'CESE'], ['FG', 'FERIADO GANADO'],
        ['LD', 'LICENCIA POR DEFUNCION'], ['DC', 'DIA COMPENSADO'], ['AP', 'ASISTENCIA PROYECTADA'],
        ['LP', 'LICENCIA PATERNIDAD'], ['TC', 'TERMINO DE CONTRATO'],
    ];
    const TITULO_ESTADO = Object.fromEntries(ESTADOS);
    const ESTADOS_DOMINGO = ['DT', 'DC', 'FT'];
    const ESTADOS_HABILITAN = ['A', 'DT', 'FT', 'DC'];   // estados que permiten pasajes/viáticos/ruta
 
    // ------------------------------------------------------------------------
    // CONFIGURACIÓN POR ÁREA  (clave = id de la <div class="section">)
    //   sufijo       -> ids del HTML: mes<sufijo>, fecha<sufijo>, nuevo-empleado-selector<sufijo>
    //   api          -> URLs: /cargar-asistencia<api>, /guardar-asistencia-detalle<api>, /eliminar-asistencia<api>
    //   modulo       -> nombre para /registrar-modulo
    ...
    const AREAS = {
        ...
        lecturas:         { sufijo: '-lecturas',         api: '-lectura',          modulo: 'asistencias_lecturas', btnAgregar: 'agregar-empleado-btn-lectura' },
        distribucion:     { sufijo: '-distribucion',     api: '-distribucion',     modulo: 'asistencias_distribucion' },
        catastro:         { sufijo: '-catastro',         api: '-catastro',         modulo: 'asistencias_catastro' },
    ...
    const UI = {};              // estado por área: { choices }
    const cargaActual = {};     // evita que una carga vieja pise a una nueva
    let catalogoPromise = null; // /añadir-empleados se pide UNA sola vez para las 9 áreas
    ...
    function enSegundoPlano(promesa) {
        promesa.catch((err) => console.warn('Petición secundaria falló:', err));
    }
 
    function diaSemana(fechaStr) {
        return fechaStr ? new Date(fechaStr + 'T00:00:00').getDay() : null;
    }
 
    function nombreVisual(nombre) {
        return typeof window.formatearNombreVisual === 'function' ? window.formatearNombreVisual(nombre) : (nombre || '');
    }
 
    function renumerar(tbody) {
        Array.from(tbody.children).forEach((fila, i) => {
            const td = fila.querySelector('td:first-child');
    ...
    function refs(area) {
        ...
        return {
            ...
            mes: $('mes' + s),
            fecha: $('fecha' + s),
            tbody: sec ? sec.querySelector('table tbody') : null,
            selector: $('nuevo-empleado-selector' + s),
            btnAgregar: $(cfg.btnAgregar || ('agregar-empleado-btn' + s)),
            btnGuardar: sec ? sec.querySelector('.smart-btn-guardar-principal') : null,
        };
    }
 
    function obtenerCatalogo() {
        if (!catalogoPromise) {
            catalogoPromise = fetch('/añadir-empleados')
                .then((r) => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
                .catch((err) => { catalogoPromise = null; throw err; });
        }
    ...
    // FILA DE LA TABLA
    // ------------------------------------------------------------------------
    function opcionesEstado(cfg, dia, seleccionado) {
        const actual = (seleccionado || '').trim();
        const codigos = dia === 0 ? [...(cfg.domingo || ESTADOS_DOMINGO)] : ESTADOS.map((e) => e[0]);
        if (actual && !codigos.includes(actual)) codigos.push(actual);   // nunca perder un estado ya guardado
        const estilo = 'style="color: #1e293b; font-weight: 500; background: #ffffff;"';
        let html = `<option value=" " ${estilo} title="Sin Asignar"> </option>`;
        codigos.forEach((c) => {
            html += `<option value="${c}" ${estilo} title="${esc(TITULO_ESTADO[c] || c)}"${c === actual ? ' selected' : ''}>${c}</option>`;
        });
        return html;
    }
 
    function crearFila(area, emp, numero, esExtra, dia) {
        const { cfg } = refs(area);
        const esPR = emp.pasajes === 'PR';
        const pasajes = !esPR && emp.pasajes ? emp.pasajes : '';
 
        const tr = document.createElement('tr');
        tr.dataset.idEmpleado = String(emp.id_empleado);
        if (esExtra) tr.dataset.extra = '1';
 
        tr.innerHTML = `
            <td style="color: #94a3b8; font-weight: bold; font-size: 14px;">${numero}</td>
            <td><span style="background: #f8fafc; padding: 4px 8px; border-radius: 4px; font-family: monospace; color: #64748b;">${esc(emp.dni)}</span></td>
            <td style="color: #0369a1; font-weight: 700;">${esc(emp.nombre_visual || nombreVisual(emp.nombres))}</td>
            <td style="color: #64748b; font-size: 12px; letter-spacing: 0.3px;">${esc(emp.cargo)}</td>
            <td style="width: 130px;">
                <select name="estado">${opcionesEstado(cfg, dia, emp.estado)}</select>
                <input type="hidden" name="id_empleado" value="${esc(emp.id_empleado)}">
            </td>
            <td style="width: 170px;">
                <div class="ui-merged-group">
                    <input type="number" name="pasajes" class="pasajes-input" step="0.01" min="0" placeholder="0.00"
                        value="${esc(pasajes)}" ${esPR ? 'disabled' : ''} style="width: 60%;">
                    <select name="pasajes" class="pasajes-select" style="width: 40%; padding-left: 5px !important;">
                        <option value="">Sel.</option>
                        <option value="PR" ${esPR ? 'selected' : ''}>PR</option>
                    </select>
                </div>
            </td>
            <td style="width: 110px;">
                <div style="position: relative;">
                    ...
                    <input type="number" name="viaticos" step="0.01" min="0" placeholder="0.00" value="${esc(emp.viaticos || '')}" style="padding-left: 30px !important;">
                </div>
            </td>
            <td><input type="text" name="ruta" placeholder="Destino..." value="${esc(emp.ruta || '')}"></td>
            <td style="text-align: center; width: 60px;">
                <button type="button" class="eliminar-fila-btn js-eliminar ui-btn-delete-icon" title="Eliminar Registro">
                    <i class="far fa-trash-alt"></i>
                </button>
            </td>`;
 
        const sEstado = tr.querySelector('select[name="estado"]');
        const iPasajes = tr.querySelector('.pasajes-input');
        const sPasajes = tr.querySelector('.pasajes-select');
        const iViaticos = tr.querySelector('input[name="viaticos"]');
        const iRuta = tr.querySelector('input[name="ruta"]');
 
        // 🔹 Auditoría de cambios (mismo formato que antes)
        const auditar = (el, tipo, { anterior = true, trim = false } = {}) => {
            let previo = el.value;
            el.addEventListener('change', () => {
                const nuevo = trim ? el.value.trim() : el.value;
                if (nuevo === previo) return;
                const body = { id_empleado: emp.id_empleado, tipo, nuevo_valor: nuevo };
                if (anterior) body.valor_anterior = previo;
                previo = nuevo;
                enSegundoPlano(postJSON('/auditar-cambio-pasajes', body));
            });
        };
        auditar(sEstado, 'estado');
        auditar(iPasajes, 'monto', { trim: true });
        auditar(sPasajes, 'select', { anterior: false });
        auditar(iViaticos, 'viaticos');
        auditar(iRuta, 'ruta', { trim: true });
 
        // 🔹 "PR" bloquea el monto
        sPasajes.addEventListener('change', () => {
            if (sPasajes.value === 'PR') { iPasajes.value = ''; iPasajes.disabled = true; }
            else { iPasajes.disabled = false; }
        });
 
        // 🔹 Solo ciertos estados permiten pasajes / viáticos / ruta
        const habilitan = 'habilitan' in cfg ? cfg.habilitan : ESTADOS_HABILITAN;
        if (habilitan) {
            const aplicar = () => {
                const ok = habilitan.includes(sEstado.value.trim());
                [iPasajes, iViaticos, iRuta].forEach((c) => { c.disabled = !ok; if (!ok) c.value = ''; });
                if (ok && sPasajes.value === 'PR') iPasajes.disabled = true;
            };
            sEstado.addEventListener('change', aplicar);
            aplicar();
        }
 
        tr.querySelector('.js-eliminar').addEventListener('click', () => eliminarFila(area, tr, emp));
        return tr;
    }
    ...
    // ACCIONES
    // ------------------------------------------------------------------------
    async function cargarEmpleados(area) {
        const r = refs(area);
        const fechaStr = r.fecha.value;
        if (!fechaStr) { alert('Seleccione una fecha válida.'); return; }
 
        const token = (cargaActual[area] = (cargaActual[area] || 0) + 1);
        r.tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; color:#94a3b8; padding:20px;">Cargando...</td></tr>';
 
        try {
            const resp = await fetch(`/cargar-asistencia${r.cfg.api}?fecha=${encodeURIComponent(fechaStr)}`);
            ...
            // 🔒 Defensa: una sola fila por empleado
            const vistos = new Set();
            const empleados = (Array.isArray(data.datos) ? data.datos : []).filter((e) => {
            ...
            if (!empleados.length) { alert('No hay empleados sin asistencia para esta fecha.'); return; }
 
            empleados.forEach((e) => { e.nombre_visual = nombreVisual(e.nombres); });
            ...
            r.tbody.innerHTML = '';
            console.error(`[${area}] Error al cargar empleados:`, err);
            alert('Ocurrió un error al cargar los empleados.');
        }
    }
 
    async function agregarExtra(area) {
        const r = refs(area);
        const ui = UI[area];
        const valor = ui.choices ? ui.choices.getValue(true) : r.selector.value;
        if (!valor) { alert('Seleccione un empleado válido.'); return; }
 
        let catalogo;
        try { catalogo = await obtenerCatalogo(); }
        catch (err) { alert('No se pudieron cargar los empleados.'); return; }
 
        const emp = catalogo.find((e) => String(e.id_empleado) === String(valor));
        if (!emp) { alert('Empleado no válido.'); return; }
 
        const yaEsta = Array.from(r.tbody.querySelectorAll('tr[data-id-empleado]'))
            .some((tr) => tr.dataset.idEmpleado === String(emp.id_empleado));
        if (yaEsta) { alert('El empleado ya está en la tabla.'); return; }
 
        const fila = crearFila(area, { ...emp, nombre_visual: nombreVisual(emp.nombres) },
            r.tbody.querySelectorAll('tr[data-id-empleado]').length + 1, true, diaSemana(r.fecha.value));
        r.tbody.appendChild(fila);
        renumerar(r.tbody);
 
        enSegundoPlano(postJSON('/auditar-agregar-empleado', { id_empleado: emp.id_empleado }));
    }
 
    async function eliminarFila(area, tr, emp) {
        const r = refs(area);
 
        // Extra aún no guardado: solo se quita de la tabla
        if (tr.dataset.extra === '1') { tr.remove(); renumerar(r.tbody); return; }
 
        const fechaStr = r.fecha.value;
        if (!confirm(`¿Estás seguro de eliminar a ${emp.nombres} de la asistencia del ${fechaStr}?`)) return;
 
        try {
            const resp = await postJSON(`/eliminar-asistencia${r.cfg.api}`, { id_empleado: emp.id_empleado, fecha: fechaStr });
            const data = await resp.json().catch(() => ({}));
            // 404 = nunca se guardó (hoja nueva): igual se quita de la tabla
    ...
    async function guardarAsistencia(area) {
        const r = refs(area);
        const filas = Array.from(r.tbody.querySelectorAll('tr[data-id-empleado]'));
        if (!filas.length) { alert('No hay empleados añadidos.'); return; }
        ...
        const asistencias = [];
        filas.forEach((f) => {
            const id = f.dataset.idEmpleado;
            if (vistos.has(id)) return;            // nunca enviar el mismo empleado dos veces
            vistos.add(id);
 
            const estado = (f.querySelector('select[name="estado"]').value || '').trim();
            const sPasajes = f.querySelector('.pasajes-select');
            const iPasajes = f.querySelector('.pasajes-input');
            let pasajes = 0;
            if (sPasajes && sPasajes.value === 'PR') pasajes = 'PR';
            else if (iPasajes && iPasajes.value.trim() !== '') pasajes = parseFloat(iPasajes.value) || 0;
            const ruta = (f.querySelector('input[name="ruta"]').value || '').trim();
            const viaticos = parseFloat(f.querySelector('input[name="viaticos"]').value) || 0;
 
            if (!estado && pasajes === 0 && viaticos === 0 && !ruta) return;   // fila vacía
 
            asistencias.push({
                mes: r.mes.value, fecha: fechaStr, estado, pasajes, ruta, viaticos,
                id_empleado: id, es_extra: f.dataset.extra === '1',
            });
        });
 
        if (!asistencias.length) { alert('No hay asistencias registradas para guardar.'); return; }
 
        if (r.btnGuardar) r.btnGuardar.disabled = true;   // evita doble clic = doble envío
        try {
            ...
            // Los extras que se guardaron ya son registros normales (eliminar => borra en BD)
            const guardados = new Set(asistencias.map((a) => a.id_empleado));
            filas.forEach((f) => { if (guardados.has(f.dataset.idEmpleado)) delete f.dataset.extra; });
        } catch (err) {
            console.error(`[${area}] Error al guardar:`, err);
            alert('Fallo de conexión con el servidor.');
        } finally {
    ...
    function initArea(area) {
        const r = refs(area);
        if (!r.sec || !r.fecha || !r.tbody) return;        // esta página no tiene esa sección
 
        UI[area] = { choices: null };
        if (r.selector && typeof window.Choices === 'function') {
            UI[area].choices = new window.Choices(r.selector, {
    ...
    async function llenarSelectores() {
        const conChoices = Object.values(UI).filter((ui) => ui.choices);
        if (!conChoices.length) return;
        try {
            const catalogo = await obtenerCatalogo();
            const opciones = catalogo
                .map((e) => ({ value: e.id_empleado, label: nombreVisual(e.nombres), customProperties: { dni: e.dni, cargo: e.cargo } }))
                .sort((a, b) => a.label.localeCompare(b.label));
            conChoices.forEach((ui) => {
                ui.choices.clearChoices();
                ui.choices.setChoices(opciones.map((o) => ({ ...o })), 'value', 'label', false);
            });
        } catch (err) {
            console.error('Error al cargar el selector de empleados:', err);
    ...
    // ------------------------------------------------------------------------
    // COMPATIBILIDAD con código existente que llame a las funciones antiguas
    // ------------------------------------------------------------------------
    window.Asistencias = { AREAS, cargarEmpleados, guardarAsistencia, agregarExtra };
    const nombresAntiguos = {
        recaudacion: 'Recaudacion', lecturas: 'Lecturas', distribucion: 'Distribucion',
        catastro: 'Catastro', inspecciones: 'Inspecciones', medidores: 'Medidores',
        persuasivas: 'Persuasivas', norte: 'Norte', administrativo_1: 'administrativo_1',
    };
    Object.entries(nombresAntiguos).forEach(([area, n]) => {
        window['cargarEmpleados' + n] = () => cargarEmpleados(area);
    });
    if (typeof window.actualizarNumeracion !== 'function') window.actualizarNumeracion = renumerar;
    if (typeof window.esDomingo !== 'function') window.esDomingo = (f) => new Date(f).getDay() === 0;
})();
        ...
        try {
            ...
            if (panelAlertas) {
                if (alertas.length === 0) {
                    ...
                    // 🔎 Desglose para verificar el cálculo
                    console.table(alertas.map((a, i) => ({
                        "#": i + 1,
                        "Desde (fin)": tarjetasCreadas[a.indiceDestino - 1].trabajo["HORA"],
                        "Hasta (ini)": tarjetasCreadas[a.indiceDestino].trabajo["HORA INI"],
                        "Actividad": tarjetasCreadas[a.indiceDestino].trabajo["ACTIVIDAD"],
                        "Espera (min)": a.minutos,
                        "Límite (min)": a.umbral,
                        "Exceso (min)": a.minutos - a.umbral,
                        "Suministro": tarjetasCreadas[a.indiceDestino].trabajo["SUMINISTRO"]
                    })));
                    console.log(`🧮 [TOTALES] Espera acumulada: ${esperaTotal} min | `
                              + `Exceso sobre el límite (tiempo perdido): ${minutosPerdidos} min`);

                    panelAlertas.innerHTML = `
                        <div class="alerta-barra">
                            <div class="alerta-resumen"
                                 title="Suma del tiempo transcurrido entre registros en las paradas que superan el límite (10 min entre lecturas, 30 min en otras actividades).&#10;Solo el excedente sobre el límite: ${formatoLargo(minutosPerdidos)}">
                                ...
                                <span class="ar-perdido">${formatoLargo(esperaTotal)}</span>
